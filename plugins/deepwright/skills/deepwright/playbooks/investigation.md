...
1. Invoke `$how` (Explain mode for narrow questions, Critique mode for "are we sure?"). For motivation questions, also invoke `$why`.
...
No writes, PR, or babysit for a read-only request. Do not invoke `$architect` unless the user asks to continue into a code change. If the investigation suggests a change, report it and wait for authorization before routing to Bug fix or Feature.
