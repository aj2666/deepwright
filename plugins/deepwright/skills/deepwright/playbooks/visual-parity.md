...
3. Migrate one component at a time. Each is an independent artifact, so parallelize across worktrees when host capacity allows, one owner per component ([Separate Before Serializing Shared State](../../principle-separate-before-serializing-shared-state/SKILL.md)). Shared primitives migrate first as a blocking phase. If collaboration is unavailable, run components sequentially.
