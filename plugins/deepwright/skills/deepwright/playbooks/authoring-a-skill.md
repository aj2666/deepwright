...
1. Use `$skill-creator` when its instructions are available within the current request's permissions. If the skill, its source, or a required tool is unavailable, follow the accessible repository conventions and Codex skill schema, and state that limitation. Do not report a skill as applied merely because its name or metadata is visible. A planning-only request still stops at the plan.
   Resolve the authorized owner before choosing a path. Update an existing skill in place. For an addition to a named plugin, use that plugin's manifest-declared skills directory and preserve its invocation policy, UI metadata conventions, and callers. Use the existing project-local skill convention for a requested project-local addition. Do not redirect a distributed skill into `.agents/skills/`, create a second copy, or install it globally as an authoring side effect.
