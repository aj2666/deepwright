...
- `decisions.tsv`: the [Show Me Your Work](../../show-me-your-work/SKILL.md) trail.
...
Each state file has one writer. Workers return facts; the coordinator updates shared tables. Files stay plain text so a cold-start host session can resume without a custom runtime.
...
1. **Frame.** State a countable done predicate, unit estimate, dependency graph, verification bar, available collaboration capacity, wall-clock or session budget, and authorization matrix. If one agent can finish within the budget, use Autonomous run instead. For a contested decomposition or one-way door, invoke [Arena](../../arena/SKILL.md).
2. **Initialize after execution authorization.** Create the run ledger, record the resolved default branch and current git/GitHub state, invoke [Show Me Your Work](../../show-me-your-work/SKILL.md), and write standing orders before assignments. A read-only planning pass stops before this step.
