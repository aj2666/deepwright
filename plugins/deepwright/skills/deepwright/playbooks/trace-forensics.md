...
1. Identify the format and load it with an available parser. When host collaboration tools are available, parse large artifacts in a read-only subagent; otherwise process them in bounded chunks. Keep the reduced finding in the main thread.
2. Transform the raw artifact into a form you can query. Put derived SQLite data under `.deepwright/runs/<run-slug>/derived/` or a unique host-provided temporary directory, keep it out of version control, and return its exact path. Reach the queryable shape before you read.
