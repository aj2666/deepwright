...
4. Implement directly when coordination would add more overhead than value. When delegation is available, authorized, and useful, give each worker a specific scope: file paths, the named data shape and organizing structure per `$deepwright:principle-model-the-domain`, and checkable success criteria. Include the authoritative acceptance criteria and their identifiers, compatibility constraints, verification target, and authorization limits in every brief. Use `$deepwright:tdd` for one behavioral slice at a time; disclose impractical checks under its fallback rules. Review the diff yourself. Use `$deepwright:arena` when competing approaches need an experiment to settle a consequential tradeoff. If collaboration is unavailable, implement sequentially and reserve a separate review pass; do not stall waiting for a capability the host does not expose. Comments follow the repository's policy. Make surgical edits, re-ground against the source for upstream-derived files, port shared-primitive improvements to every consumer, and verify each.
