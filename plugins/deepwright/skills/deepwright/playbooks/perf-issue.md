...
1. Capture a baseline trace with the host's available browser, terminal, simulator, or application-control capability. If none can reach the surface, state the gap and use the closest reproducible harness; do not invent evidence.
2. Invoke [How](../../how/SKILL.md) to ground hypotheses; do not claim a performance ceiling without measuring it first.
   ...
   - **Elimination.** The cheapest work is work that does not run. Before optimizing the hot path, ask whether it needs to exist: a computation nobody consumes, a feature gate that is always off for this user, a sync that redundantly mirrors state, or a legacy path kept "just in case". The trace shows what is slow, never that it is deletable, so this family needs the [How](../../how/SKILL.md) pass, not only the profiler. Deleting the work beats every other family when it applies.
...
3. Plan the fix from the trace. If it crosses a function boundary, invoke [Architect](../../architect/SKILL.md) first. When host collaboration tools are available, delegate implementation to a subagent with a bounded scope and acceptance criteria, then review the diff. Otherwise implement sequentially and perform a separate review pass. Capture a post-fix trace with the same harness.
   Apply [Sequence Verifiable Units](../../principle-sequence-verifiable-units/SKILL.md), verifying each attempt before trying the next.
4. Parse and compare the artifacts (JSON to sqlite, diff). "Inconclusive" or wrong-surface is not a pass; flag it.
5. When a PR is authorized, cite the measurement and artifact in its description.
