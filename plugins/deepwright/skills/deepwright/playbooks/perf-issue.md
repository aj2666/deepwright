...
2. Invoke `$deepwright:how` to ground hypotheses; do not claim a performance ceiling without measuring it first.
...
3. Plan the fix from the trace. If it crosses a function boundary, invoke `$deepwright:architect` first. When host collaboration tools are available, delegate implementation to a subagent with a bounded scope and acceptance criteria, then review the diff. Otherwise implement sequentially and perform a separate review pass. Capture a post-fix trace with the same harness.
   Apply `$deepwright:principle-sequence-verifiable-units`, verifying each attempt before trying the next.
