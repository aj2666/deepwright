...
export function assessGitHubMerge(args: {
  ...
  readonly exactHeadObserved: boolean;
  ...
  // BLOCKED is GitHub's aggregate branch-policy result. A passing commit
  // rollup proves only that the visible checks passed; it does not prove that
  // every required review, deployment, ruleset, or merge-queue gate passed.
  // Until this watcher reads and proves those policies explicitly, fail closed.
  const draftOverride =
    args.allowDraft && args.isDraft && args.mergeStateStatus === "DRAFT";
  if (args.mergeStateStatus !== "CLEAN" && !draftOverride)
    return {
      kind: "refused",
      reason:
        args.mergeStateStatus === "BLOCKED"
          ? "merge-state-blocked"
          : "merge-state-unproven",
      exactHead: "observed",
      mergeStateStatus: args.mergeStateStatus,
      headRollupState: args.headRollupState,
    };
...
async function mergeAssessment(
  ...
  const headCommit =
    facts.headRefOid === null
      ? undefined
      : commits.find((commit) => commit.oid === facts.headRefOid);
  ...
  return {
    ...
    github: assessGitHubMerge({
      ...
      headRollupState,
      exactHeadObserved: headCommit !== undefined,
...
function assertConsistentLifecycle(facts: T.PullRequestFacts): void {
...
function assertRequestedContext(
  ...
  if (
    facts.context.owner !== context.owner ||
    facts.context.repo !== context.repo ||
    facts.context.number !== context.number
  )
    throw new WatcherQueryError({
      kind: "missing-key",
      retryable: true,
...
function gateReason(
  ...
  const acceptedDraftState =
    allowDraft && row.facts.isDraft && row.facts.mergeStateStatus === "DRAFT";
...
async function pollUntilTerminal<V>(args: {
  ...
  while (true) {
    ...
    try {
      ...
      const timeoutVerdict = (): T.TimeoutVerdict =>
        args.stamp({
          kind: "TIMEOUT",
          terminal: true,
          exitCode: 5,
          ...
          onDeadline: () =>
            stamp({
              ...
              reason: {
                ...
                frontier: evaluation.frontier,
                unmergedCount:
                  evaluation.reason.kind === "merge-queue"
