...
// @ts-expect-error BLOCKED is a refusal even when the visible rollup passes.
...
const reviewRequiredPr = {
  ...
  proof: {
    ...readyPr.proof,
    gate: { ...readyPr.proof.gate, reviewDecision: "REVIEW_REQUIRED" },
  },
} as const;

// @ts-expect-error READY cannot carry a still-required review decision.
const readyWithRequiredReview: ReadyPr = reviewRequiredPr;

void refusalIsNotAllowed;
void refusalIsNotClean;
void readyWithBlockerExit;
void readyWithoutProof;
void readyWithRequiredReview;
