...
The first follow-up macOS CI run caught a second fixture assumption: creating `FEATURE.MD` replaced the existing `feature.md` on case-insensitive APFS instead of creating an extra file. The test now uses the distinct basename `UNCATALOGUED.MD`, preserving its uppercase-extension rejection check and all assertions; runtime behavior did not change.

The final small integration pass connects every direct configuration consumer to that same reference, including How, Interrogate, Arena, Reflect, and Show me your work. Direct invocation therefore uses the same validation and user/project/default precedence as routed invocation. It removes repeated configurable defaults while retaining fixed workflow counts, and corrects two leaves to inherit the selected parent model rather than an unspecified host default. Package validation also requires both bundled dependency licenses and their NOTICE references; isolated negative checks reject either missing artifact. This completes the recommended architecture work without introducing a new router, state store, configuration hierarchy, or adapter engine.
